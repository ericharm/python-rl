...
class Entity:
    ...
    def move(self, x, y, level):
        ...
        if (
            destination.x < (level.width - 1)
            and destination.y < (level.height - 1)
...
class Hero(Entity):
    ...
    def get_zaps(self):
        return reduce(
            lambda a, b: a if a["name"] == "Zapgun Charges" else b, self.inventory
        )
...
class Enemy(Entity):
    ...
    def move(self, hero, level):
        ...
        my_tile = reduce(
            lambda a, b: a if a.x is self.x and a.y is self.y else b, tiles
        )
        hero_tile = reduce(
            lambda a, b: a if a.x is hero.x and a.y is hero.y else b, tiles
        )
