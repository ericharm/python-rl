...
class Tile:
    ...
    def get_empty_at_distance_two(self, tiles):
        tiles_at_distance_two = list(
            filter(lambda tile: tile.at_distance(2, self), tiles)
        )
        empty_at_distance_two = list(
            filter(lambda tile: tile.empty(), tiles_at_distance_two)
        )
        return empty_at_distance_two

    def at_distance(self, n, target):
        if target.x is self.x:
            return target.y is self.y - n or target.y is self.y + n
        elif target.y is self.y:
            return target.x is self.x - n or target.x is self.x + n
        else:
            return False
    ...
    def dead_end(self, adjacents):
        ...
        return (
            len(empties) is (len(adjacents) - 1)
            and len(walkables) == 1
            and self.type == "corridor"
        )
